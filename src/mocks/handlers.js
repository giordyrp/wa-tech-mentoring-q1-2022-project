...
export const handlers = [
  rest.get(
    ...
    (req, res, ctx) => {
      ...
      if (documentType.includes('product')) {
        ...
        const filterQuery = req.url.searchParams
          .getAll('q')
          .find((elem) => elem.includes('any'));
        if (filterQuery)
          data.results = data.results.filter((product) =>
            filterQuery.includes(product.data.category.id)
          );

        const page = req.url.searchParams.get('page');
        if (page) data.page = parseInt(page);

        const productId = req.url.searchParams
          .getAll('q')
          .find((elem) => elem.includes('id'));
        if (productId)
          data.results = data.results.filter((product) => productId.includes(product.id));

        let searchTerm = req.url.searchParams
          .getAll('q')
          .find((elem) => elem.includes('fulltext'));
        if (searchTerm) {
          const regex = new RegExp('.*, "(.*)"\\x29.*');
          searchTerm = regex.exec(searchTerm)[1];
          data.results = data.results.filter((product) =>
            product.data.name.includes(searchTerm)
          );
        }
  ...
  rest.get('https://wizeline-academy.cdn.prismic.io/api/v2', (req, res, ctx) => {
    return res(
      ...
      ctx.json({
        refs: [
          { id: 'master', ref: 'YZaBvBIAACgAvnOP', label: 'Master', isMasterRef: true },
        ],
      })
    );
  }),
